...
fn read_startup_launch_enabled<R: Runtime>(app: &AppHandle<R>) -> Result<bool, StartupLaunchError> {
    app.autolaunch()
        .is_enabled()
...
mod tests {
    ...
    fn startup_launch_error_messages_are_not_empty() {
        ...
        let disable_error = StartupLaunchError::Disable(String::from("disable failed")).to_string();
